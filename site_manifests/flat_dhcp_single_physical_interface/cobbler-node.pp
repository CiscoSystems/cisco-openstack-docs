...
$apt_proxy_addr = '192.168.100.254'
$physical_boot_disk = '/dev/sda'
$vlan_id = '400'
...
# }
 ...
 class { cobbler:
  ...
  domain_name => "sdu.lab",
  proxy => "http://${apt_proxy_addr}:3142/",
...
echo "auto eth0.${vlan_id}" >> /target/etc/network/interfaces ; \
echo "iface eth0.${vlan_id} inet loopback" >> /target/etc/network/interfaces ; \
...
',
  ...
  expert_disk => true,
  diskpart => ["${physical_boot_disk}],
  boot_disk => "${physical_boot_disk},
 }
